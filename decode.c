...
int decode_start(char * filename, video_callback* video_callback(uint8_t *data, int size, int64_t pts), audio_callback* audio_callback(uint8_t *data, int size, int64_t pts)){
	if (!init){
		decode_init();
	}
	...
	while (1)
	{
		...
		if (i_pkt.stream_index == video_stream_index) {
			...
			if (got_picture) {
				...
				video_callback(i_pkt.data, i_pkt.size, i_pkt.pts);
			...
			//音频流
				...
				if (got_picture) {
					...
					audio_callback(i_pkt.data, i_pkt.size, i_pkt.pts);
				}
				else{
